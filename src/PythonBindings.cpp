...
PythonScript::PythonScript() { LOG_DEBUG("Constructing PythonScript: {:p}", fmt::ptr(this)); }
PythonScript::~PythonScript() { LOG_DEBUG("Destructing PythonScript: {:p}", fmt::ptr(this)); }

void PythonScript::runtime_begin() { }
void PythonScript::update(double /* dt */) { }
void PythonScript::runtime_end() { }
void PythonScript::editor_update() { }

PythonScriptBinding::PythonScriptBinding()
    : PythonScript()
{
    LOG_DEBUG("Constructing PythonScriptBinding: {:p}", fmt::ptr(this));
}

PythonScriptBinding::PythonScriptBinding(PythonScript&& base)
    : PythonScript(std::move(base))
{
    LOG_DEBUG("Constructing PythonScriptBinding: {:p}", fmt::ptr(this));
}

PythonScriptBinding::~PythonScriptBinding() { LOG_DEBUG("Destructing PythonScriptBinding: {:p}", fmt::ptr(this)); }
void PythonScriptBinding::runtime_begin() { PYBIND11_OVERRIDE(void, PythonScript, runtime_begin); }
void PythonScriptBinding::update(double dt) { PYBIND11_OVERRIDE(void, PythonScript, update, dt); }
void PythonScriptBinding::runtime_end() { PYBIND11_OVERRIDE(void, PythonScript, runtime_end); }
void PythonScriptBinding::editor_update() { PYBIND11_OVERRIDE(void, PythonScript, editor_update); }
...
PYBIND11_EMBEDDED_MODULE(peanut, m)
{
    ...
    py::class_<PythonScript, PythonScriptBinding>(m, "PythonScript")
        ...
        .def_property("transform", &PythonScript::get<TransformComponent>, &PythonScript::set<TransformComponent>,
            py::return_value_policy::reference)
        .def_readwrite("editor_fields", &PythonScript::editor_fields, py::return_value_policy::reference);

    m.def("is_key_pressed", [](int i) { return PEANUT::Input::IsKeyPressed(static_cast<KeyCode>(i)); });
