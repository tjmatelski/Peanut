...
template <class T>
void Serialize(YAML::Emitter&, const T&, std::filesystem::path)
{
    assert(false && "Missing specialization for component");
}

void Serialize(YAML::Emitter& out, const PEANUT::TagComponent& tag, std::filesystem::path)
{
...
void Serialize(YAML::Emitter& out, const PEANUT::TransformComponent& transform, std::filesystem::path)
{
...
void Serialize(YAML::Emitter& out, const PEANUT::SpriteRenderComponent& spriteRenderComp, std::filesystem::path)
{
...
void Serialize(YAML::Emitter& out, const PEANUT::ModelFileComponent& comp, std::filesystem::path)
{
...
void Serialize(YAML::Emitter& out, const PEANUT::DirectionalLightComponent& light, std::filesystem::path)
{
...
void Serialize(YAML::Emitter& out, const PEANUT::PointLightComponent& light, std::filesystem::path)
{
...
void Serialize(YAML::Emitter& out, const PEANUT::SkyboxComponent& comp, std::filesystem::path)
{
...
void Serialize(YAML::Emitter& out, std::pair<PEANUT::NativeScript*, std::string> p, std::filesystem::path)
{
...
template <class T>
void Deserialize(T&, YAML::Node, std::filesystem::path)
{
    assert(false && "Missing deserialization specialization");
}

void Deserialize(PEANUT::TagComponent& comp, YAML::Node node, std::filesystem::path)
{
    comp.tag = node["Tag"].as<std::string>();
}

void Deserialize(PEANUT::TransformComponent& comp, YAML::Node node, std::filesystem::path)
{
...
void Deserialize(PEANUT::ModelFileComponent& comp, YAML::Node node, std::filesystem::path)
{
    std::filesystem::path relative(node["File"].as<std::string>());
    comp.file = PEANUT::Settings::GetResourceDir() / relative;
}

void Deserialize(PEANUT::DirectionalLightComponent& comp, YAML::Node node, std::filesystem::path)
{
...
void Deserialize(PEANUT::PointLightComponent& comp, YAML::Node node, std::filesystem::path)
{
    comp.color = node["Color"].as<glm::vec3>();
    comp.ambient = node["Ambient"].as<float>();
    comp.diffuse = node["Diffuse"].as<float>();
...
void Deserialize(PEANUT::SkyboxComponent& comp, YAML::Node node, std::filesystem::path)
{
...
template <class T>
void Deserialize(const char* name, PEANUT::Entity ent, YAML::Node node, std::filesystem::path projectDir, const std::vector<std::string>&)
{
    if constexpr (std::is_same_v<T, PEANUT::NativeScript>) {
    } else if (node[name]) {
