...
{
    return std::make_unique<FileSelectorDialog>();
}

std::optional<std::string> FileSelectorDialog::OpenFile()
{
    return SelectFile(Gtk::FILE_CHOOSER_ACTION_OPEN);
}

std::optional<std::string> FileSelectorDialog::SaveFile()
{
    return SelectFile(Gtk::FILE_CHOOSER_ACTION_SAVE);
}

std::optional<std::string> FileSelectorDialog::OpenDirectory()
{
